...
static const int portNameSize = 32;

static inline void
portToName(int port, char* buf)
{
    *(buf++) = '/';
    *(buf++) = 'v';
    *(buf++) = 'a';
...
int
XmlRpcSocket::socket(bool local)
{
    initWinSock();
    return (int) ::socket(local ? AF_UNIX : AF_INET, SOCK_STREAM, 0);
}
...
bool
XmlRpcSocket::bind(int fd, int port, bool local)
{
    if(local)
    {
        ...
        char path[portNameSize];
        portToName(port, path);
...
bool
XmlRpcSocket::connect(int fd, std::string& host, int port, bool local)
{
    if(local)
    {
        struct sockaddr_un saddr;
        ...
        struct sockaddr_in saddr;
        memset(&saddr, 0, sizeof(saddr));
        saddr.sin_family = local ? AF_UNIX : AF_INET;
