...
XmlRpcClient::XmlRpcClient(const char* host, int port, bool local, const char* uri/*=0*/)
{
...
{
    int fd = XmlRpcSocket::socket(_local);
    if(fd < 0)
    {
    ...
    if(! XmlRpcSocket::connect(fd, _host, _port, _local))
    {
        this->close();
...
XmlRpcClient::parseResponse(XmlRpcValue& result)
{
    ...
    TiXmlElement* methodresponse = doc->FirstChildElement("methodResponse");
    ...
    if(TiXmlElement* methodparams = methodresponse->FirstChildElement("params"))
    {
        TiXmlElement* methodparam = methodparams->FirstChildElement("param");
        if(methodparam == NULL)
        {
            XmlRpcUtil::error("Error in XmlRpcClient::parseResponse: Invalid response - no param tag. Response:\n%s", _response.c_str());
            return false;
        }
        result.fromXml(methodparam->FirstChildElement());
    } else if(TiXmlElement* methodfault = methodresponse->FirstChildElement("fault"))
    {
        result.fromXml(methodfault->FirstChildElement());
        _isFault = true;
    } else {
