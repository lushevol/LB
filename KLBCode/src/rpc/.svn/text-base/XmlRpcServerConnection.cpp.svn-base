...
#ifdef __SHOW_XML_RPC__
            std::auto_ptr<TiXmlDocument> doc(new TiXmlDocument());
            doc->Parse(resultValue.toXml().c_str());
            std::cout << "Response " << __FILE__ << " " << __LINE__ << ":" << std::endl;
            doc->Print();
#endif
...
{
    std::auto_ptr<TiXmlDocument> doc(new TiXmlDocument());
    doc->Parse(_request.c_str());
#ifdef __SHOW_XML_RPC__
    std::cout << __FILE__ << " " << __LINE__ << ":" << std::endl;
    doc->Print();
#endif
    TiXmlElement* methodcall = doc->FirstChildElement("methodCall");
    if(methodcall == NULL)
        throw XmlRpcException("Unknown methodCall tag.");

    TiXmlElement* methodname = methodcall->FirstChildElement("methodName");
    if(methodname == NULL)
        throw XmlRpcException("Unknown methodName tag.");

    const char* name = methodname->GetText();
    if(!name || *name == 0)
        return "";
    if(TiXmlElement* methodparams = methodcall->FirstChildElement("params"))
    {
        int nArgs = 0;
        TiXmlNode* child = NULL;
        while((child = methodparams->IterateChildren(child)))
        {
            if(TiXmlElement* methodparam = child->ToElement())
            {
                if(methodparam->ValueStr() != "param")
                    throw XmlRpcException("Unknown param tag.");
                params[nArgs++].fromXml(methodparam->FirstChildElement());
            }
        }
    }
    return name;
