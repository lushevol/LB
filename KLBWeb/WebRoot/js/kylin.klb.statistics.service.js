...
if (!window.kylin.klb.service) {
	window.kylin.klb.statistics.service = {};
}
...
$(document).ready(function() {
	...
	kylin.klb.statistics.service.showChart();
	...
	$("#statisExport").click(function(e) {
		e.stopPropagation();
		kylin.klb.statistics.service.excel();
		return false;
	});
});
...
(function(package) {
	jQuery.extend(package, {
		showChart : function() {
			...
			$.ajax({
				type: "POST",
				url: "klb-statistics-service!initServer.action",
				dataType : 'json',
				timeout : 10000,
							...
							if ($.trim(server) == "") {
								...
								for (var i = 0;i <= max; i++) {
									...
									if ($.trim(val) != "") {
										...
										var chartSrc = "klb-statistics!service.action?" + param
												+ "&r=" + Math.random();
								...
								var chartSrc = "klb-statistics!service.action?" + param + "&r="
										+ Math.random();
								img.push("<img id=\"statisticsid\" src=\"" + chartSrc
										+ "\" alt=\"\" width=\"763\" />");
							}
		...
		excel: function(){
			...
			if ($.trim(server) != "") {
				var chartSrc = "klb-st-service.action?r="
						+ Math.random();
