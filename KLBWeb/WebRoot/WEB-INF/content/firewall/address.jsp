...
<script type="text/javascript" src="${ctx}/js/firewall/address.js"></script>
</head>
<body>
<!--移动弹出层开始-->
<div id="mark"></div>
...
<div class="mainPartContNoTop">
  ...
  <div class="txtCont">
    ...
    <p class="vLine"></p>
    <div class="box3"><h3><span>源地址转换</span></h3></div>
	<br />	
	<!--列表开始-->
		...
		<div class="ct">
			...
			<div class="main">
				...
				<div class="tableList">
					<table>
						<caption>
							<span class="tit"><strong>源地址转换信息列表</strong></span>
							...
							<span class="operate">
								<a href="address-src!list.action?operation=add&id=-1" class="addLink popDivBt" id="add">添加</a>
							</span>
						</caption>
						<thead>
							<tr>
						...
						<tbody>
							<s:iterator value="natList" id="natList" status="index">
								<tr>
									...
									<td>${interfaces}</td>
									<td>
										<s:if test="%{type == 'EXPT'}">不作转换</s:if>
										<s:if test="%{startIP == 'masq'}">动态IP</s:if>
										<s:elseif test="%{startIP != 'masq'}">${startIP}</s:elseif>
									...
									<%-- <td><a href="javascript:;" class="move" onclick="address.recordOldId({'id':'${id}'})">
										<img src="${ctx}/css/skin/images/common/ico_move.gif" alt="移动" title="移动" /></a></td>--%>
									<td><a href="address-src!list.action?operation=insert&id=${id}">
									...
									<td>
										<span>										
											<s:if test="%{enabled == 'true'}">
												<a href="javascript:;" onclick="address.start(this,{'id':'<s:property value="#index.index" />','enabled':'false'})">
												关闭</a>
											</s:if>
											<s:else>
												<a href="javascript:;" onclick="address.start(this,{'id':'<s:property value="#index.index" />','enabled':'true'})">
												开启</a>
											</s:else>
										</span>
										<a href="address-src!list.action?operation=edit&id=${id}">设置</a>
										<span><a href="javascript:;" onclick="address.del(this,{'id':'${id}'})">删除</a></span>
									</td>
								</tr>
							</s:iterator>
						</tbody>
